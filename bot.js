    ...
    if (message.startsWith("!request")) {
      ...
      if (message.length > 1) {
        if (message[1].indexOf("soundcloud.com") > -1) {
          ...
          if (message[1].substring(0, 1) == "/")
            message[1] = "https://soundcloud.com" + message[1];
      ...
      client.say(
        channel,
        "Listen directly to the channel of the streamer at: https://zoff.me/" +
...
function update_moderate(enabled, channel) {
  ...
  dbase.channels.update(
    { channel: channel },
    { $set: { moderate: enabled } },
    function(err, chan) {
      var toSay = enabled
        ? "Now moderating your channel, blocking/allowing urls"
        : "Not moderating your channel";
...
function handleOwnChannel(user, message, secure) {
  ...
  if (message.startsWith("!join")) {
    ...
    client.say(
      "#zoffbot",
      "Joined your channel " + user.username.replace("#", "")
    );
  } else if (message.startsWith("!leave")) {
    leaveChannel(channel, user, secure);
  }
}
...
function leaveChannel(channel, user, secure) {
  ...
  if (secure) {
    client.whisper(
      channel,
      "Left your channel " + user.username.replace("#", "")
    );
    return;
  }
...
function message_users(channel) {
  ...
  var messages = [
    "Check out Zoff at https://zoff.me, and create your own channel!",
    "Listen directly to this Zoff channel at https://zoff.me/" +
      config.channels[channel]
  ];
...
function fetch_next_playing(channel) {
  dbase.channels.find({ channel: channel }, function(err, chan) {
    if (chan.length == 0) {
      client.say(
        channel,
        "Couldn't fetch now playing, are you sure there is something in the list?"
      );
      return;
    }
          ...
          request(data, function(err, response, body) {
            ...
            if (json.status == 200) {
              ...
              if (json.results.length > 1) {
                ...
                client.say(
                  channel,
                  "Couldn't find next song for some reason.."
                );
                return;
              }
              client.say(channel, 'Next song: "' + nextTitle + '"');
              ...
              client.say(
                channel,
                "Couldn't fetch next song, are you sure there is something in the list?"
              );
            }
          });
        }
    ...
    request(data, function(err, response, body) {
      ...
      try {
        ...
        if (json.status == 406) {
          ...
          client.say(
            channel,
            "You have already voted to skip on this song " + username + "."
          );
          return;
          ...
          client.say(
            channel,
            json.results[0] + " more are needed to skip the song!"
          );
          return;
        }
      } catch (e) {
        ...
        if (json.results[0].source == "youtube") {
          ...
          request(
            ...
            function(err, response, body) {
              ...
              client.say(
                channel,
                'Now playing: "' + json.results[0].title + '"'
              );
              client.say(channel, "Link: " + link);
            }
          );
  ...
  dbase.channels.find({ channel: channel }, function(err, docs) {
    if (docs[0].hasOwnProperty("moderate") && docs[0].moderate) {
      if (config.allowed[channel].indexOf(from) < 0) {
        ...
        config.allowed[channel].splice(
          config.allowed[channel].indexOf(from),
          1
        );
      }
    }
...
function send_help(channel, message) {
  if (message === undefined || message == "!help") {
    client.say(
      channel,
      "To request help with any commands, please say !help COMMAND. Available commands are: " +
        commands.join(", ")
    );
  } else {
    switch (message.substring(6)) {
      case "request":
        client.say(
          channel,
          "This is used to request a song. If you type !request YOUTUBE_ID (change the YOUTUBE_ID with an actual YouTube ID) or !request SOUNDCLOUD_DATA (the text after 'soundcloud(dot)com/'), you'll request a song on the Zoff channel the streamer is listening on."
        );
        break;
      case "np":
        client.say(
          channel,
          "This tells the currently playing song on the Zoff channel of the streamer."
        );
        break;
      case "skip":
        client.say(
          channel,
          "This is to vote to skip the currently playing song. The channel has to be set up with strict skipping."
        );
        break;
      case "settime":
        client.say(
          channel,
          "This is only for mods, and it sets the streamers timezone in GMT. Use with !settime +TIME"
        );
        break;
      case "time":
      ...
      case "channel":
        client.say(
          channel,
          "This sends the channel name the streamer is currently listening to."
        );
        break;
      case "allow":
        client.say(
          channel,
          "This is only for mods, and it allows a specific user to send one link. Use with !allow NAME"
        );
        break;
      case "moderate":
        client.say(
          channel,
          "This is only for mods, and sets me to either moderate or not moderate url posting"
        );
        break;
      case "next":
...
function join_channels() {
  dbase.channels.find({ zoffchannel_initialized: { $ne: false } }, function(
    err,
    docs
  ) {
    for (var x in docs) {
...
function settime(time_from, channel) {
  ...
  var add = "";
  if (isNaN(time))
    client.say(channel, '"' + time_from + "\" isn't a number.. Try again");
  else {
    if (time > 0) add = "+";
    client.say(
      channel,
      channel.substring(1) + "s timezone has been changed to GMT" + add + time
    );
      ...
      request(
        ...
        function(err, response, body) {
          try {
            ...
            client.say(
              twitch_channel,
              "Couldn't add song, sure the channel is set-up?"
            );
          }
        }
      );
    ...
    if (chan.length > 0) {
      request(
        "https://www.googleapis.com/youtube/v3/videos?id=" +
          id +
          "&part=contentDetails,snippet,id&key=" +
          secrets.key,
        function(err, response, body) {
          try {
            ...
            callback(
              { id: id, title: title, duration: duration, source: "youtube" },
              chan[0],
              twitch_channel,
              requester
            );
...
function add_song(song_info, channel, twitch_channel, requester) {
  ...
  var url =
    "https://zoff.me/api/list/" + channel.zoffchannel + "/" + song_info.id;
  var addObject = {
  ...
  if (song_info.source == "soundcloud")
    addObject.thumbnail = song_info.thumbnail;
  var data = {
    uri: url,
    url: url,
    form: addObject,
  ...
  request(data, function(err, response, body) {
    ...
    if (json.status == 200) {
      client.say(
        twitch_channel,
        requester + ' added "' + song_info.title + '"'
      );
    } else if (json.status == 403) {
      client.say(
        twitch_channel,
        requester +
          ' suggested "' +
          song_info.title +
          '". It needs an admin to accept it to be valid.'
      );
    } else if (json.status == 409) {
      addObject = {
        adminpass: "",
      ...
      if (addObject.source == "source")
        addObject.thumbnail = song_info.thumbnail;
      request.put(
        {
          url:
            "https://zoff.me/api/list/" +
            channel.zoffchannel +
            "/" +
            song_info.id,
          form: addObject
